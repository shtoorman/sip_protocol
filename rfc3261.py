...
#                self.__dict__[n.lower().strip()] = _unquote(v.strip())
    ...
    def createHeaders(value):
        ...
        return (_canon(name),  [Header(re.sub(r'%2C', r',', x), name) for x in value.split(',') if name.lower() not in _comma else [value]])
...
class Message(object):
    ...
    def __iter__(self):
        ...
        for n in [x for x in self.__dict__ if not x.startswith('_') and x not in Message._keywords]:
            h += [x for x in self[n] if isinstance(self[n],list) else [self[n]] if isinstance(x, Header)]
        return iter(h)
